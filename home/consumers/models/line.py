...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "Chicago.transport.faust.stations": # stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic().startswith("Chicago.transport.stations"): # arrival topic
            self._handle_arrival(message)
        elif message.topic() == "turnstile_summary": #  KSQL Turnstile Summary Topic
